...
export const authResolvers = {
  ...
  Mutation: {
    ...
    adminCreateUser: async (_p, { input }, ctx) => {
      requireRole(ctx, PLATFORM_ROLES);
      const { role, name, email, password, phone, city, tenantId } = input;

      if (['FRANCHISE_OWNER', 'FRANCHISE_STAFF'].includes(role) && !tenantId) {
        throw new GraphQLError('tenantId is required for franchise roles.', { extensions: { code: 'BAD_USER_INPUT' } });
      }

      const existing = await db('users').whereRaw('lower(email) = lower(?)', [email]).whereNull('deleted_at').first();
      if (existing) throw new GraphQLError('An account with this email already exists.', { extensions: { code: 'CONFLICT' } });
      ...
      const [user] = await db('users').insert({
        tenant_id: ['FRANCHISE_OWNER', 'FRANCHISE_STAFF'].includes(role) ? tenantId : null,
      ...
      await db('audit_logs').insert({
        tenant_id: user.tenant_id, actor_id: ctx.user.id, actor_role: ctx.user.role,
        action: 'user.created_by_admin', entity_type: 'user', entity_id: user.id,
      });

      return user;
    },

    adminUpdateUser: async (_p, { id, input }, ctx) => {
      requireRole(ctx, PLATFORM_ROLES);
      const patch = {};
      ...
      if (input.tenantId !== undefined) patch.tenant_id = input.tenantId;

      const [updated] = await db('users').where('id', id).update(patch).returning('*');
      if (!updated) throw new GraphQLError('User not found.', { extensions: { code: 'NOT_FOUND' } });

      await db('audit_logs').insert({
        tenant_id: updated.tenant_id, actor_id: ctx.user.id, actor_role: ctx.user.role,
        action: 'user.updated_by_admin', entity_type: 'user', entity_id: id, after_state: patch,
      });
    ...
    deleteUser: async (_p, { id }, ctx) => {
      requireRole(ctx, PLATFORM_ROLES);
      const [updated] = await db('users').where('id', id)
        .update({ deleted_at: db.fn.now(), is_active: false }).returning('id');
      if (!updated) throw new GraphQLError('User not found.', { extensions: { code: 'NOT_FOUND' } });

      await db('audit_logs').insert({
        actor_id: ctx.user.id, actor_role: ctx.user.role,
        action: 'user.deleted_by_admin', entity_type: 'user', entity_id: id,
      });
